...
def professional_dashboard(id):
    ...
    open_service_requests = db.session.query(ServiceRequest, User, Service).join(
        User, ServiceRequest.customer_id == User.id
    ).join(
        Service, ServiceRequest.service_id == Service.id
    ).filter(
        ServiceRequest.status == "requested", ServiceRequest.professional_id == id
    ).add_columns(
        ServiceRequest.id, ServiceRequest.date_of_request, User.name.label("customer_name"), Service.name.label("service_name")
    ).all()

    assigned_service_requests = db.session.query(ServiceRequest, User, Service).join(
        User, ServiceRequest.customer_id == User.id
    ).join(
        Service, ServiceRequest.service_id == Service.id
    ).filter(
        ServiceRequest.status == "assigned", ServiceRequest.professional_id == id
    ).add_columns(
        ServiceRequest.id, ServiceRequest.date_of_request, User.name.label("customer_name"), Service.name.label("service_name")
    ).all()

    closed_service_requests = db.session.query(ServiceRequest, User, Service, Review).join(
        User, ServiceRequest.customer_id == User.id
    ).join(
        Service, ServiceRequest.service_id == Service.id
    ).outerjoin(
        Review, ServiceRequest.id == Review.servicereq_id
    ).filter(
        ServiceRequest.status == "closed", ServiceRequest.professional_id == id
    ).add_columns(
        ServiceRequest.id, ServiceRequest.date_of_request,ServiceRequest.date_of_completion, User.name.label("customer_name"), 
        Service.name.label("service_name"), Review.rating, Review.comment.label("review")
    ).all()
